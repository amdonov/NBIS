...
/***********************************************************************/
int
parse_line_range (const char *sb, int *begin, int *end)
{
  int ib, ie;
  char *se;
...
/***********************************************************************/
void
set_progname (int use_pid, char *basename, pid_t pid)
{
  if (use_pid)
...
/***********************************************************************/
void
set_probe_filename (char *filename)
{
  pfile = filename;
}
...
/***********************************************************************/
void
set_gallery_filename (char *filename)
{
  gfile = filename;
}
...
get_next_file (char *fixed_file,
	       ...
	       char *line,
	       int argc,
	       char **argv, int *optind, int *lineno, int begin, int end)
{
  char *p;
  FILE *fp;
  ...
  if (fp != (FILE *) NULL)
	  ...
	  if (*lineno > end)
	    {
	      *done_now = 1;
	      if (verbose_main)
		fprintf (errorfp,
			 "returning NULL -- current line (%d) > end line (%d)\n",
			 *lineno, end);
	      return (char *) NULL;
	    }
	  if (*lineno >= begin)
      ...
      if (p == (char *) NULL)
	{
	  *done_now = 1;
	  if (verbose_main)
	    fprintf (errorfp,
		     "returning NULL -- missing newline character\n");
	  return (char *) NULL;
	}
      *p = '\0';
...
char *
get_score_filename (const char *outdir, const char *listfile)
{
  const char *basename;
  int baselen;
  int dirlen;
...
/* extern char * get_progname( void ); */
  ...
  outfile =
    malloc_or_return_error (dirlen + baselen + extlen + 2, "output filename");
  if (outfile == CNULL)
    return (CNULL);
...
/***********************************************************************/
char *
get_score_line (const char *probe_file,
		const char *gallery_file,
		int n, int static_flag, const char *fmt)
{
  int nchars;
  char *bufptr;
...
bz_load (const char *xyt_file)
{
  ...
  int xvals_lng[MAX_FILE_MINUTIAE],	/* Temporary lists to store all the minutaie from a file */
    ...
    tvals_lng[MAX_FILE_MINUTIAE], qvals_lng[MAX_FILE_MINUTIAE];
  ...
  if (fp == (FILE *) NULL)
    {
      fprintf (errorfp,
	       "%s: ERROR: fopen() of minutiae file \"%s\" failed: %s\n",
  ...
  while (fgets (xyt_line, sizeof xyt_line, fp) != CNULL)
    {
      m = sscanf (xyt_line, "%d %d %d %d",
		  &xvals_lng[nminutiae],
		  &yvals_lng[nminutiae],
		  &tvals_lng[nminutiae], &qvals_lng[nminutiae]);
      ...
      if (nminutiae == 0)
	{
	  if (m != 3 && m != 4)
	    {
	      fprintf (errorfp,
		       "%s: ERROR: sscanf() failed on line %u in minutiae file \"%s\"\n",
      ...
      else
	{
	  if (m != nargs_expected)
	    {
	      fprintf (errorfp,
		       "%s: ERROR: inconsistent argument count on line %u of minutiae file \"%s\"\n",
		       get_progname (), nminutiae + 1, xyt_file);
	      return XYT_NULL;
	    }
	}
  ...
  if (fclose (fp) != 0)
    {
      fprintf (errorfp,
	       "%s: ERROR: fclose() of minutiae file \"%s\" failed: %s\n",
	       get_progname (), xyt_file, strerror (errno));
      return XYT_NULL;
    }
  ...
  if (xytq_s == XYTQ_NULL)
    {
      fprintf (errorfp,
	       "%s: ERROR: malloc() failure while loading minutiae buffer failed: %s\n",
...
*************************************************************************/
struct xyt_struct *
bz_prune (struct xytq_struct *xytq_s, int verbose_load)
{
  ...
  int xvals[MAX_BOZORTH_MINUTIAE],
    ...
    tvals[MAX_BOZORTH_MINUTIAE], qvals[MAX_BOZORTH_MINUTIAE];
  char xyt_line[MAX_LINE_LENGTH];
...
#define C2 1
  ...
  if (nminutiae > max_minutiae)
    {
      if (verbose_load)
	fprintf (errorfp,
		 "%s: WARNING: bz_prune(): trimming minutiae to the %d of highest quality\n",
      ...
      for (j = 0; j < nminutiae; j++)
	{
	  ...
	  if (qvals_lng[order[j]] > qvals_lng[order[j - 1]])
	    {
	      fprintf (errorfp,
		       "%s: ERROR: sort failed: j=%d; qvals_lng[%d] > qvals_lng[%d]\n",
      ...
      if (C1)
	{
	  ...
	  for (j = 0; j < nminutiae; j++)
		...
		{
		  fprintf (errorfp,
			   "%s: ERROR: sort failed: c[%d].col[3] > c[%d].col[3]\n",
			   get_progname (), j, j - 1);
		  return XYT_NULL;
		}
	    }
  ...
  qsort ((void *) &c, (size_t) nminutiae, sizeof (struct minutiae_struct),
	 sort_x_y);
  ...
  if (verbose_load)
      ...
      for (j = 0; j < nminutiae; j++)
	{
	  fprintf (errorfp, "%d : %3d, %3d, %3d, %3d\n", j, c[j].col[0],
		   c[j].col[1], c[j].col[2], c[j].col[3]);
	  if (j > 0)
	    {
	      if (c[j].col[0] < c[j - 1].col[0])
		{
		  fprintf (errorfp,
			   "%s: ERROR: sort failed: c[%d].col[0]=%d > c[%d].col[0]=%d\n",
			   get_progname (), j, c[j].col[0], j - 1,
			   c[j - 1].col[0]);
		  return XYT_NULL;
		}
	      if (c[j].col[0] == c[j - 1].col[0]
		  && c[j].col[1] < c[j - 1].col[1])
		{
		  fprintf (errorfp,
			   "%s: ERROR: sort failed: c[%d].col[0]=%d == c[%d].col[0]=%d; c[%d].col[0]=%d == c[%d].col[0]=%d\n",
			   get_progname (), j, c[j].col[0], j - 1,
			   c[j - 1].col[0], j, c[j].col[1], j - 1,
			   c[j - 1].col[1]);
		  return XYT_NULL;
		}
	    }
  ...
  if (xyt_s == XYT_NULL)
    {
      fprintf (errorfp, "%s: ERROR: malloc() failure while xyt_struct.");
      return XYT_NULL;
    }
